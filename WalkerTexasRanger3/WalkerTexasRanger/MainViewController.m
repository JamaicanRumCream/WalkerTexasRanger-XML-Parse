...
- (IBAction)startProcess:(id)sender
{
    ...
    sizesArray = [NSMutableArray array];
    stylesArray = [NSMutableArray array];
    
    [parser parse];
    
    [rootObject logChildren]; //log the tree structure when parse is done
    NSLog(@"sizesArray = %@", sizesArray);
    NSLog(@"stylesArray = %@", stylesArray);
}
...
/*
 ...
 next the first element is found.
 a new data object is made and then added to the children of the root item (or previous item)
 ...
 the flat array is used to keep track of what the last element is (elementArray)
 */
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    if (!rootObject) {
        ...
        MyData *newObject = [[MyData alloc] init];
        [newObject setName:elementName];
        [[[elementArray lastObject] children] addObject:newObject];
        [elementArray addObject:newObject];
        
        //look for any elements with string "style" in the name
        if ([elementName rangeOfString:@"style" options:NSCaseInsensitiveSearch].length != 0) {
            [stylesArray addObject:newObject];
        }
    }
    captureChars = YES;
}
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    //put just the value of one particular item into an array
    if ([elementName isEqualToString:@"size1"]) {
        [sizesArray addObject:[currentElementValue copy]];
    }
    
    //look for any elements with string "style" in the name
    if ([elementName rangeOfString:@"style" options:NSCaseInsensitiveSearch].length != 0) {
        if (captureChars == YES) {
            //if we are starting/appending an element; on closing this value is NO
            //otherwise this statement will clear the previous value in the last object
            [[stylesArray lastObject] setDataContent:[currentElementValue copy]];
        }
    }
